...
def process_multiple_images(input_images):
...
def run(params):
    ...
    print(f"Saved decoded_traffic_dos/traffic_{rounds}.txt")
...
# def save_to_txt(dataset, file_path, data_csv_file):

#     def convert_label(raw_label, pkt_label):
...
#         # raw_label "None" + pkt_label == 1 → A
#         if raw_label == "None" and pkt_label == "1":
#             return "A"

#         # everything else → B
#         return "B"

#     with open(file_path, 'w') as file, open(data_csv_file, 'r') as csv_file:
...
#         # Validate required columns exist
#         required_cols = ["timestamp", "can_id", "label", "perturbation_type"]
#         for col in required_cols:
#             if col not in col_index:
#                 raise KeyError(f"Column '{col}' not found in CSV header: {header}")
...
#             # Use column names instead of hardcoded [-3], [-2], etc.
#             timestamp = float(extra_data[col_index["timestamp"]])
#             pkt_label = extra_data[col_index["label"]]               # old 'label'
#             raw_label = extra_data[col_index["perturbation_type"]]   # raw attack label (I/M/None)

#             final_label = convert_label(raw_label, pkt_label)
...
# def process_multiple_images(input_images, output_file, csv_file):

#     if input_images == "dos_k12":
#         image_folder = r"perturbed_images"
#     elif input_images == "test80":
#         image_folder = r"test80/perturbed"
#     else:
#         print("Invalid input. Please provide a valid filetype.")
#         return
...
#     save_to_txt(all_data, output_file, csv_file)
...
#     process_multiple_images(input_images, output_file, csv_file)
#     print("Decoded")
